...
class BertForTokenClassification(BertPreTrainedModel):
    def __init__(self, config):
        ...
        if config.add_pos:
            ##### ADD POS #####
            self.emission = nn.Linear(config.hidden_size+self.pos_num_labels-1, config.num_labels)
        else:
    ...
    Examples::
        ...
        if self.add_pos:
            ...
            sequence_output = torch.cat([sequence_output, pos_one_hot], 2)
        # print('sequence_output:', sequence_output.size())

        logits = self.emission(sequence_output) # (batch_size, seq_len, num_labels)
